...
Those three actions are also three bounded contexts: the meme, the comment and the vote. A vote is cast on both memes and comments, so a context map is needed from day one. Favourites came later and grew into collections. That list explains the size of the account deletion saga further down: every context is one more place to clean up.
